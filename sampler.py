...
class Sampler(nn.Module):
    ...
    def forward(
        ...
        # Use float32 for the logits (fall back to native dtype on XPU
        # where bfloat16 -> float32 cast is unsupported: UR error 45).
        target_dtype = torch.bfloat16 if logits.device.type == "xpu" else torch.float32
    ...
    def apply_temperature(
        ...
        # Move temp to the same device and dtype as logits to avoid:
        # 1. XPU UR error 45 from mixed device (CPU temp vs XPU logits)
        # 2. XPU UR error 45 from mixed dtype (float32 temp vs bfloat16 logits)
    ...
    def compute_logprobs(logits: torch.Tensor) -> torch.Tensor:
        # Use float32 for numerical stability (bfloat16 on XPU: UR error 45).
        logprob_dtype = torch.bfloat16 if logits.device.type == "xpu" else torch.float32
        return logits.log_softmax(dim=-1, dtype=logprob_dtype)
