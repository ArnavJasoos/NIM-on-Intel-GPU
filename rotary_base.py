...
class RotaryEmbeddingBase(CustomOp):
    ...
    def __init__(
        ...
        # Move to XPU if available. The cache is computed on CPU to avoid
        # a driver-level FP64 crash (UR error: 45) on Intel iGPUs during
        # the base**arange exponentiation, but it must reside on the same
        # device as the query tensor before torch.compile traces index_select.
        import torch
